...
class Outside_controller extends CI_Controller {
  ...
  function __construct(){
	...
	$data['arr'] = explode(",", $_REQUEST['tag']);
	$output['school_name'] =$data[0];
	$data = (explode(" ",$data[1]));
	$output['school_code']=($data[2]);
	
	$data= array('school_code' =>$output['school']['school_code'],
	...
	function assesment_school(){
	    ...
	    $result = $this->db->query("SELECT b.school_code,b.school_name FROM assessment a , school b WHERE school_name LIKE '%$q%' AND a.assessment_status != 1 group by b.school_code ORDER BY b.id ASC LIMIT 0,$limit")->result_array();
	    $customers = array();
