...
class Teacher_test extends CI_Controller 
{
 function __construct(){
	...
	$this->load->model('teachertest_model');
 ...
 function add_tenth(){
    $schooltenth =$this->teachertest_model->add_tenth_model($this->arr['school_code']);
...
Author : Nishant Sharma*/
  ...
  function input($value){
	$this->arr['action'] = $value;
	 if($value = "insert"){
	$this->teachertest_model->general($this->arr);
	$this->arr['message'] = 'Data has been added in data base';
	$data['arr'] =$this->teachertest_model->display_form($this->arr);
	$data1['setvalues'] =$this->teachertest_model->drop_down($this->arr);
	$this->load->view('ttsheet',array_merge($this->arr , $data,$data1));	
	 }
...
function update()
	...
	$this->teachertest_model->update_query($this->arr);
...
Author : Nishant Sharma*/
  ...
  function form($action){
    ...
    $this->load->view('ttsheet',array_merge($this->arr , $data,$data1));
	/*if(($this->arr['action'] == 'action') && ($_REQUEST['numbofrow'] > 0)){
	
	}
	$data['arr'] =$this->teachertest_model->display_form($this->arr);
	$	
  if (isset($_REQUEST['numbofrow'])){
	$data['arr'] =$this->teachertest_model->display_form($this->arr);
  }*/
...
$data['arr'] =$this->teachertest_model->display_form($this->arr);
$data1['setvalues'] =$this->teachertest_model->drop_down($this->arr);
$this->load->view('ttsheet_result',array_merge($this->arr , $data,$data1));
}
...
$data1['setvalues'] =$this->teachertest_model->drop_down($this->arr);
$this->load->view('ttsheet',array_merge($this->arr , $data1));
