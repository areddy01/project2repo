...
function admissions()
	...
	{
		$this->schoolfee_model->general($this->info);
	...
	function transport ()
	 ...
	 function addtransportfees ()
		 {
		 	$this->transportfee_model->general($this->arr);
			$data =$this->transportfee_model->display_form($this->arr);
			$data ['arr'] =  $data;
			$this->load->view('transportfees',array_merge($this->arr,$data));	
	...
	function addattendance ()
	 ...
	 function getform_data(){
		...
		$value['arr'] = $output;
		$result['category']= $this->db->query("SELECT B.survey_cat_name,B.survey_cat_id FROM `ref_survey_category` AS B")->result_array();
