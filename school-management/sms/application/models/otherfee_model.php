  ...
  function general($school){
	...
	for($incrementnext= 0 ; $incrementnext < count($_REQUEST['other_fees_values'][$increment]); $incrementnext++){
		...
		$data = array('assessment_id'=>$school['assessment_id'], 
			...
			'fee_amount' =>$_REQUEST['other_fees_values'][$increment][$incrementnext]['fee_amount'], 
			'quantity' =>$_REQUEST['other_fees_values'][$increment][$incrementnext]['quantity'],
			'total_amount'=>'',
		...
		//echo "SELECT * FROM `other_fees` a ,`assessment` b  WHERE a.school_code = '".$data['school_code']."' AND a.class_id =".$data['class_id']." and a.fee_id=".$data['fee_id']." AND b.assessment_id = a.assessment_id  AND b.assessment_status =1";
...
function display_form($school){
	...
	$setvalues = $this->db->query("SELECT a.fee_id,a.fee_amount,a.quantity FROM `other_fees` a ,`assessment` b  WHERE a.school_code = '".$school['school_code']."' AND b.assessment_id = a.assessment_id  AND b.assessment_status =1");
