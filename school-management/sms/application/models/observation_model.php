...
function display_form($school){
	$setvalues = $this->db->query("SELECT ref_observ_sub_category.observ_sub_cat_id,ref_observ_category.observ_cat_name, ref_observ_category.observ_cat_id, ref_observ_sub_category.observ_sub_cat_name  FROM `ref_observ_category` INNER JOIN `ref_observ_sub_category` ON ref_observ_category.observ_cat_id = ref_observ_sub_category.observ_cat_id ORDER BY ref_observ_sub_category.observ_cat_id ");
