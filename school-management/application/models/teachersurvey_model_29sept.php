...
Author : Nishant Sharma */
  ...
  function general($school){
	...
	for($increment= 0 ; $increment < count($_REQUEST['survey_sub_cat_id']);$increment++){
		...
		$recsetclear = $this->db->query("SELECT teacher_survey_id FROM `teacher_survey` a ,`assessment` b  WHERE a.school_code = '".$data['school_code']."' AND b.assessment_id = a.assessment_id  AND b.assessment_status =1 AND survey_sub_cat_id  = '".$data['survey_sub_cat_id']."' AND response = '".$data['response']."'");
...
function display_form($school){
	...
	$getvalue = $this->db->query("SELECT * FROM `teacher_survey` a ,`assessment` b  WHERE a.school_code = '".$school['school_code']."' AND b.assessment_id = a.assessment_id  AND b.assessment_status =1 AND response ='".$_REQUEST['response'][0]."'");
	}
	}
	else
	{
	$getvalue = $this->db->query("SELECT * FROM `teacher_survey` a ,`assessment` b  WHERE a.school_code = '".$school['school_code']."' AND b.assessment_id = a.assessment_id  AND b.assessment_status =1 ");
	}
