...
echo $_REQUEST['name'];
echo $_REQUEST['class_taught'];echo $_REQUEST['subject_taught'];
	...
	//echo "SELECT observation_id FROM `observation` a ,`assessment` b  WHERE a.school_code = '".$data['school_code']."' AND b.assessment_id = a.assessment_id  AND b.assessment_status =1 AND observ_sub_cat_id  = '".$data['observ_sub_cat_id']."' AND response = '".$data['response']."'";
...
function display_form($school){
	...
	$formarray1['setvalues'] =($setvalues->result_array()); //echo $_REQUEST['response'][0]; //PRINT_R($formarray1['setvalues']);
	//$formarray2['getvalues'] =($getvalues->result_array());
	if (isset($_REQUEST['response'][0])){
	if (($_REQUEST['response'][0] != null )){	
	
		$getvalue = $this->db->query("SELECT DISTINCT(response)  FROM `observation` a ,`assessment` b  WHERE a.school_code = '".$school['school_code']."' AND b.assessment_id = a.assessment_id  AND b.assessment_status =1 ");
	}
	
	else
	{
	//SELECT * FROM `observation` a ,`assessment` b  WHERE a.school_code = '".$school['school_code']."' AND b.assessment_id = a.assessment_id  AND b.assessment_status =1"
	$getvalue = $this->db->query("SELECT  DISTINCT(response) FROM `observation` a ,`assessment` b  WHERE a.school_code = '".$school['school_code']."' AND b.assessment_id = a.assessment_id  AND b.assessment_status =1");
	}
