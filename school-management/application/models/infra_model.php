  ...
  function general($school){
	 ...
	 $j++;
	   ...
	   $assesment_field =array('school_code' => $school['school_code'],
		...
		$school_details= array('school_code'=>$school['school_code'],'school_name'=>$school['school_name']);
		$school_details= array_merge($school_details,$_REQUEST['school_profile']);
	  ...
	  if(count($validate) != 0){
	   ...
	   // $this->db->where($condition);
		...
		$condition = array('school_code' => $school['school_code']);
	    $this->db->where($condition);
		$this->db->update('school_profile' ,$school_details);
	  }
		else{
		//$condition = array('school_code' => $school['school_code']);
		//$this->db->insert('assessment', $assesment_field);
		$this->db->insert('school_profile' , $school_details);
...
//function categories(){
//$catege = $this->db->query("SELECT facility_cat_id FROM ref_facility_category ");
	//	$catege =($catege->result_array());
//return $catege;
//}
...
function display_form($school){
	...
	$data = $this->db->query("select `assessment_id`, `school_code`, `academic_year`, `assessment_date`, `assessment_status`, `assessment_term_type`, `school_assessed_by`, `no_of_section`, `school_assessed_again_same_year`, `assessment_type_if_re_assessed`, `aps_Average` from assessment where school_code = '".$school['school_code']."' and assessment_status = 1");
...
LEFT JOIN ref_facility_category ON ref_facility_sub_category.facility_cat_id = ref_facility_category.facility_cat_id ORDER BY ref_facility_category.facility_cat_id");
			...
			$formarray2=array_merge($formarray,$data);
			//echo "<pre>";
			//print_r($formarray2);
			//exit;
			return $formarray2;
