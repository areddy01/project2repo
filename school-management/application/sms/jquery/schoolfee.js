function validate_schfee_school(value){
			
		
				for($i=0; $i< value - 1 ;$i++){
				for($j=0;$j< 6 ; $j++) {
				
					texbox1 = $("#school_fees_val"+$i+$j).val();
					
					if ((texbox1 < 0)){
					$('#error').show();
					return	false ;
					}
					else if(isNaN(texbox1)){
					$('#error').show();
					return	false ;
					}
					}
				
		}
		$("#message").hide();
		return  true ;
}



function validate_selection(validateval,$row){

	$("#message").hide();	
	var data=$(validateval).val();
	
	for($i=0; $i< $row ;$i++){
		
	if ($('#schoolfee0').val() == 'Select'){
	
		document.getElementById('school_fees_val'+$i+0).value = '';
		$('#school_fees_val'+$i+0).attr('readonly', 'readonly');
		}
		else
		{
		$("#school_fees_val"+$i+0).removeAttr("readonly","readonly");

		}
		
			
		if (($('#schoolfee1').val()) == 'Select'){							
		document.getElementById('school_fees_val'+$i+1).value = '';	
		$('#school_fees_val'+$i+1).attr('readonly', 'readonly');	
		}
		else
		{
		$("#school_fees_val"+$i+1).removeAttr("readonly","readonly");
		}
	
		
		if (($('#schoolfee2').val()) == 'Select'){							
		document.getElementById('school_fees_val'+$i+2).value = '';	
		$('#school_fees_val'+$i+2).attr('readonly', 'readonly');	
		}
		else
		{
		$("#school_fees_val"+$i+2).removeAttr("readonly","readonly");
		
		}
	
			
		if (($('#schoolfee3').val()) == 'Select'){							
		document.getElementById('school_fees_val'+$i+3).value = '';	
		$('#school_fees_val'+$i+3).attr('readonly', 'readonly');	
		}
		else
		{
		$("#school_fees_val"+$i+3).removeAttr("readonly","readonly");
		}
	
			
		if (($('#schoolfee4').val()) == 'Select'){							
		document.getElementById('school_fees_val'+$i+4).value = '';
		$('#school_fees_val'+$i+4).attr('readonly', 'readonly');		}
		
		else
		{
		$("#school_fees_val"+$i+4).removeAttr("readonly","readonly");		}
	
		if (($('#schoolfee5').val()) == 'Select'){							
		document.getElementById('school_fees_val'+$i+5).value = '';	
		$('#school_fees_val'+$i+5).attr('readonly', 'readonly');	
		}
	
		else
		{
		$("#school_fees_val"+$i+5).removeAttr("readonly","readonly");
		}
				

	}
	return true;
	}

	