...
function getform_data(){
		...
		$result['category']= $this->db->query("SELECT B.facility_cat_name,B.facility_cat_id FROM `ref_facility_category` AS B GROUP BY facility_cat_name")->result_array();
 ...
 function submitform_data(){
		...
		//$this->arr['message'] = 'Category is added';
		$result['category']= $this->db->query("SELECT `facility_cat_name`,`facility_cat_id` FROM `ref_facility_category`")->result_array();//AND A.school_code = '".$this->arr['school_code']."'
		$this->load->view('infra_lookup', array_merge($this->session_data,$result,$this->arr));
		 }
		...
		if ($_REQUEST['formname'] == 'infra'){
		//print_r($_REQUEST['formname']);
		//exit;
		if($this->content_model->submit_content($_REQUEST)){
		$this->arr['message'] = 'Infra details are added'; 
		}
		
		$result['category']= $this->db->query("SELECT B.facility_cat_name,B.facility_cat_id FROM `ref_facility_sub_category` AS A ,`ref_facility_category` AS B WHERE A.facility_cat_id = B.facility_cat_id GROUP BY facility_cat_name")->result_array();//AND A.school_code = '".$this->arr['school_code']."'
		$this->load->view('infra_lookup', array_merge($this->arr,$result,$this->session_data));
		 }
		 else if ($_REQUEST['formname'] == 'enrollment'){ 
