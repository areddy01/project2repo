...
class Outside_controller extends CI_Controller {
  ...
  function __construct(){
	...
	if(isset($_REQUEST['submit']) && ($_REQUEST['schoolname']!="")){
	//if($_REQUEST['schoolname']!=""){
	$result = $this->db->query("SELECT school_code FROM school ORDER BY id DESC LIMIT 0,1")->result_array();
	$schoolid = $result[0]['school_code'];
	$rest =  preg_replace('/\D/', '', $schoolid); //    \D means non digit.
    $rest = $rest +1 ;
	$var =str_pad($rest, 4, '0', STR_PAD_LEFT);
    $value['school_code']  = $_REQUEST['schoolloc'].$var;
	$value['school_name'] =  $_REQUEST['schoolname'];
	...
	{
	$value['message'] = 'please write the school name';
	$this->load->view('add_school',  array_merge($value,$this->session_data));
	}
	}
    ...
    $config["base_url"] = base_url() . "outside_controller/display_school";
	...
	$data["results"] = $this->db->query("SELECT a.school_code, b.assessment_status, a.school_name FROM school a, assessment b WHERE a.school_code =b.school_code AND  a.school_code ='".$school[count($school)-1]."' GROUP BY a.school_code ORDER BY a.id")->result_array();
	$data["links"] = $this->pagination->create_links();
	...
	function assesment_school(){
	    ...
	    $result = $this->db->query("SELECT b.school_code,b.school_name FROM assessment a , school b WHERE school_name LIKE '%$q%'  group by b.school_code ORDER BY b.id ASC LIMIT 0,$limit")->result_array();
