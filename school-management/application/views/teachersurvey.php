...
<form method = "post" id ="" action = "/admin/addteachersurvey/?info=<?php echo $info ; ?>"    style="width:90%; height: 800px;">
	<table border="0" style="width: 97%">
  <tr ><!-- Row 1 -->
     <td class="auto-style3" colspan="2">
	 <div align="center" class="auto-style5"><strong>Respondent</strong></div></td><!-- Col 1 -->
      <!-- Col 2 -->
	   
     <td class="auto-style10" ><strong>SL RESPONSE</strong></td>
	 <td >
		 <strong>
		 <input type = "text" readonly = "readonly" cursor= "disabled" value = "<?php if(isset($T1)){ $T = $T1+1; echo("T$T"); }else echo "T1"; ?>" name = "response[]" size="20" style="width: 92px" class="auto-style10" />
		</strong></td>
	   </tr><!-- Row 1 --> 
...
// print_r($arr);
  ...
  <tr style="background-color:#008000"><!-- Row 1 -->
   ...
   <td class="auto-style9" style="width: 28px"><?php echo $row1++; ?></td>
     ...
     <td> 
		 <select id="teachersur<?php echo($row);?>0" name="teacher_response0[<?php echo($row);?>]" style="width: 87px" >
 ...
 <input type = "hidden" value = "<?php echo $arr['setvalues'][$row]['survey_cat_id']; ?>" name = "survey_cat_id[]" /> 
	 ...
	 <?php if($arr['setvalues'][$row]['survey_sub_cat_type'] == 'dropdown') {
	  ...
	  // } ?> 
		 <select id="teachersur<?php echo($row);?>" name="teacher_response1[<?php echo($row);?>]" value = "<?php echo $arr['setvalues'][$row]['survey_sub_cat_id']; ?>" style="width: 87px" >
			<option value="NULL" >Select</option>
			<option value="0" >0</option>
			<option value="1" >1</option>
			<option  value="2" >2</option>
...
<input type = "text" value = "" name="teacher_response1[<?php echo($row);?>" />
 <?php } ?>
...
<span><td align="right"><?php if(isset($confirm)) echo $confirm; ?></td></span></tr>
